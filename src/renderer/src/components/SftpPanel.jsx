...
export default function SftpPanel({ profile, onClose }) {
  ...
  async function deleteEntry(name, isDir) {
    ...
    if (!ok) return;
    const r = await window.nexterm.sftp.delete(
      connId,
      joinPath(path, name),
      isDir,
    );
  ...
  return (
    <div className="sftp-panel">
      <div className="sftp-header">
        ...
        <button className="icon-btn" onClick={onClose} style={{ fontSize: 18 }}>
          ×
        </button>
      </div>
      <div className="sftp-toolbar">
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter") refresh(e.target.value);
          }}
        />
        <button
          className="btn-secondary"
      ...
      <div className="sftp-list" ref={dropRef}>
        ...
        {!busy && entries.length === 0 && (
          <div className="sftp-empty">Empty</div>
        )}
        {!busy &&
          entries.map((e) => (
            <div
              key={e.name}
              className="sftp-row"
              onDoubleClick={() => navigate(e.name, e.isDir)}
            >
              <span className="sftp-icon">{e.isDir ? "📁" : "📄"}</span>
              <span className="sftp-name" title={e.longname || e.name}>
                {e.name}
              </span>
              ...
              <span className="sftp-actions">
                ...
                <button
                  className="icon-btn"
                  onClick={() => deleteEntry(e.name, e.isDir)}
                  title="Delete"
                >
                  🗑
                </button>
      ...
      <div className="sftp-hint">
        Drop files here to upload to <code>{path}</code>
      </div>
    </div>
  );
}
